...
function ExternalGame({content,journey}:{content:Record<string,unknown>;journey?:JourneyContext}) {
  ...
  return <section className="external-game-board"><header><div><p className="overline">{providerLabel(game.provider)}</p><h3>Ressource externe guidée</h3><p>{game.provider==='wordwall'?'Seule l’activité Wordwall est affichée dans Progressed Pédago.':'La ressource s’adapte à votre écran.'}</p></div><a className="button light" href={game.provider==='wordwall'?game.embedUrl:(game.sourceUrl||game.embedUrl)} target="_blank" rel="noreferrer">Ouvrir l’activité en plein écran ↗</a></header>{game.presentationImageUrl&&<img className="external-game-presentation" src={game.presentationImageUrl} alt="Présentation de la ressource externe"/>}{game.introduction&&<div className="external-game-introduction"><StructuredExplanation text={game.introduction} compact/></div>}{game.preGameExplanation&&<details className="external-game-preparation" open><summary>À savoir avant de commencer</summary><StructuredExplanation text={game.preGameExplanation} compact/>{game.learnerTips.length>0&&<ul>{game.learnerTips.map((tip)=><li key={tip}>{tip}</li>)}</ul>}</details>}{game.summary&&<details className="external-game-preparation"><summary>Résumé de la ressource</summary><StructuredExplanation text={game.summary} compact/></details>}{game.memo&&<details className="external-game-preparation"><summary>Fiche mémo</summary><StructuredExplanation text={game.memo} compact/></details>}{!journey&&game.paper.officialFileId&&<aside className="external-official-support"><div><small>Support officiel associé</small><strong>{game.paper.officialFileName||'Version imprimable fournie par le site externe'}</strong></div><a className="button light" href={`/api/files/${game.paper.officialFileId}`} target="_blank" rel="noreferrer">Télécharger</a></aside>}<div className="external-game-frame"><iframe src={game.embedUrl} title="Ressource externe intégrée" loading="lazy" sandbox={game.provider==='wordwall'?'allow-scripts allow-forms allow-same-origin allow-presentation':'allow-scripts allow-forms allow-same-origin allow-popups allow-presentation'} allow="fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/></div>{game.scenario.title&&<details className="external-game-preparation" open><summary>Mise en situation · {game.scenario.title}</summary><p>{game.scenario.context}</p><p><strong>Votre rôle :</strong> {game.scenario.learnerRole} · <strong>Interlocuteur :</strong> {game.scenario.aiRole}</p><p><strong>Mission :</strong> {game.scenario.mission}</p>{game.scenario.prompts.length>0&&<ol>{game.scenario.prompts.map((prompt)=><li key={prompt}>{prompt}</li>)}</ol>}</details>}<section className="external-game-completion"><div><h4>Après l’activité</h4>{game.debrief&&<StructuredExplanation text={game.debrief} compact/>}<p>Le service externe ne transmet pas forcément votre résultat à Progressed Pédago. Aucun résultat automatique n’est donc inventé.</p></div>{game.scoreMode==='self_report'&&<label>Mon score, facultatif<div><input type="number" min="0" max={game.scoreMax} value={score} onChange={(event)=>setScore(event.target.value)} /><span>/ {game.scoreMax}</span></div></label>}<button className="button dark" type="button" disabled={busy||completed} onClick={()=>void complete()}>{busy?'Validation…':completed?'Activité terminée ✓':'J’ai terminé cette activité'}</button>{message&&<p role="status">{message}</p>}</section></section>;
}
