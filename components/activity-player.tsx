...
import { normalizeExternalGameContent, type ExternalGamePaperOptions } from '@/lib/external-games';
...
function ExternalGame({content,journey}:{content:Record<string,unknown>;journey?:JourneyContext}) {
  ...
  return <section className="external-game-board"><header><div><p className="overline">{game.provider==='wordwall'?'Jeu Wordwall':'Activité externe intégrée'}</p><h3>Jouez directement dans l’activité</h3><p>Le jeu s’adapte à votre écran. S’il refuse l’intégration, ouvrez-le dans un nouvel onglet sans perdre votre parcours.</p></div><a className="button light" href={game.embedUrl} target="_blank" rel="noreferrer">Ouvrir séparément ↗</a></header>{game.presentationImageUrl&&<img className="external-game-presentation" src={game.presentationImageUrl} alt="Présentation du jeu externe"/>}{game.introduction&&<div className="external-game-introduction"><StructuredExplanation text={game.introduction} compact/></div>}{game.preGameExplanation&&<details className="external-game-preparation" open><summary>À savoir avant de jouer</summary><StructuredExplanation text={game.preGameExplanation} compact/>{game.learnerTips.length>0&&<ul>{game.learnerTips.map((tip)=><li key={tip}>{tip}</li>)}</ul>}</details>}{!journey&&game.paper.officialFileId&&<aside className="external-official-support"><div><small>Support officiel associé</small><strong>{game.paper.officialFileName||'Version imprimable fournie par le site externe'}</strong></div><a className="button light" href={`/api/files/${game.paper.officialFileId}`} target="_blank" rel="noreferrer">Télécharger</a></aside>}<div className="external-game-frame"><iframe src={game.embedUrl} title="Jeu externe intégré" loading="lazy" sandbox="allow-scripts allow-forms allow-same-origin allow-popups allow-presentation" allow="fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/></div><section className="external-game-completion"><div><h4>Après le jeu</h4>{game.debrief&&<StructuredExplanation text={game.debrief} compact/>}<p>Le site externe ne transmet pas forcément votre résultat à Progressed Pédago. Aucun résultat automatique n’est donc créé.</p></div>{game.scoreMode==='self_report'&&<label>Mon score, facultatif<div><input type="number" min="0" max={game.scoreMax} value={score} onChange={(event)=>setScore(event.target.value)} /><span>/ {game.scoreMax}</span></div></label>}<button className="button dark" type="button" disabled={busy||completed} onClick={()=>void complete()}>{busy?'Validation…':completed?'Activité terminée ✓':'J’ai terminé cette activité'}</button>{message&&<p role="status">{message}</p>}</section></section>;
}
