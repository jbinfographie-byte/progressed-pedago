  ...
  return <section className="scenario-board">
    ...
    <div className="scenario-layout">
      <article className="scenario-main-card">
        ...
        {validated && selected && <ScenarioFeedback choice={selected} onContinue={continueScenario} last={index === scenario.scenes.length - 1} />}
      </article>
      <aside className="scenario-side-card"><section><span>Votre mission</span><p>{scene.mission}</p></section><section><span>Objectif pédagogique</span><p>{scene.objective}</p></section><ScenarioList title="Compétences mobilisées" values={scene.competencies} /><ScenarioList title="Points à observer" values={scene.observationCriteria} /><ScenarioList title="Informations utiles" values={scene.usefulInformation} /><ScenarioList title="Documents et indices" values={scene.documents} /></aside>
    </div>
  </section>;
}

function ScenarioFeedback({ choice,onContinue,last }: { choice:ScenarioChoice;onContinue:()=>void;last:boolean }) { const status = choice.score === 2 ? 'Décision recommandée' : choice.score === 1 ? 'Décision partiellement adaptée' : 'Décision à risque'; return <section className={`scenario-feedback score-${choice.score}`} role="status"><header><span>{choice.score === 2 ? '✓' : choice.score === 1 ? '!' : '×'}</span><div><small>Retour sur votre décision · {choice.score}/2</small><h4>{status}</h4></div></header><p className="scenario-consequence"><strong>Conséquence</strong>{choice.consequence}</p><div className="scenario-feedback-columns">{!!choice.positivePoints.length && <div><strong>Points positifs</strong><ul>{choice.positivePoints.map((item) => <li key={item}>{item}</li>)}</ul></div>}{!!choice.risks.length && <div><strong>Risques</strong><ul>{choice.risks.map((item) => <li key={item}>{item}</li>)}</ul></div>}</div><p><strong>Conduite recommandée</strong>{choice.recommendedConduct}</p><p><strong>Explication pédagogique</strong>{choice.explanation}</p>{choice.source && <p className="scenario-source"><strong>Source</strong>{choice.source}</p>}<button className="button dark" type="button" onClick={onContinue}>{last ? 'Voir mon bilan' : 'Continuer la situation'} →</button></section>; }
