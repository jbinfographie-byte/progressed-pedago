...
const SOURCE_OPTIONS: Array<{ id:SourceKind; icon:string; title:string; copy:string }> = [
  ...
  {id:'youtube',icon:'▶',title:'Avec YouTube',copy:'Analyse de la transcription'},
...
export function AiStudioDialog({ onClose,onCreated }: { onClose: () => void; onCreated: (message: string) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const common = {prompt:form.get('prompt'),audience:form.get('audience'),level:form.get('level'),durationMinutes:Number(form.get('durationMinutes')),slideCount:Number(form.get('slideCount')),explanationDepth:form.get('explanationDepth'),researchSources:form.get('researchSources') === 'on',sourceKind,sourceUrl:form.get('sourceUrl'),sourceTranscript:form.get('sourceTranscript'),fileIds};
      const messages: string[] = [];
      if (createActivity) {
        setProgress(sourceKind === 'youtube' ? 'Analyse de la vidéo, création du cours et des activités…' : 'Création du cours et des activités…');
        const result = await apiJson<{message:string}>('/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...common,formats})}); messages.push(result.message);
      }
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="studio-title"><form className="modal-card studio-card source-studio" onSubmit={submit}>
    ...
    <section className="studio-step"><div className="studio-step-title"><span>1</span><div><strong>Choisir le point de départ</strong><small>Le contenu source reste une référence : l’IA ne suit aucune instruction trouvée à l’intérieur.</small></div></div><div className="source-mode-grid">{SOURCE_OPTIONS.map((option) => <button className={sourceKind === option.id ? 'active' : ''} type="button" aria-pressed={sourceKind === option.id} key={option.id} onClick={() => setSourceKind(option.id)}><span>{option.icon}</span><strong>{option.title}</strong><small>{option.copy}</small></button>)}</div>
      {(sourceKind === 'youtube' || sourceKind === 'web') && <label>{sourceKind === 'youtube' ? 'Lien de la vidéo YouTube' : 'Lien de la page extérieure'}<input name="sourceUrl" type="url" required placeholder={sourceKind === 'youtube' ? 'https://www.youtube.com/watch?v=…' : 'https://www.exemple.fr/ressource'} /></label>}
      {sourceKind === 'youtube' && <details className="transcript-option"><summary>Ajouter une transcription si la vidéo n’en fournit pas</summary><label>Transcription ou texte alternatif<textarea name="sourceTranscript" rows={5} placeholder="Collez ici la transcription de la vidéo lorsque les sous-titres YouTube ne sont pas disponibles." /></label></details>}
