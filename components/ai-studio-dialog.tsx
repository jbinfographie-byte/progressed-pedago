...
export function AiStudioDialog({ onClose,onCreated }: { onClose: () => void; onCreated: (message: string) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const common: Record<string,unknown> = {prompt:form.get('prompt'),audience:form.get('audience'),level:form.get('level'),durationMinutes:Number(form.get('durationMinutes')),slideCount:Number(form.get('slideCount')),explanationDepth:form.get('explanationDepth'),researchSources:form.get('researchSources') === 'on',sourceKind,sourceUrl:form.get('sourceUrl'),sourceTranscript:form.get('sourceTranscript'),fileIds,scenarioCount:Number(form.get('scenarioCount') || 4),scenarioDifficulty:form.get('scenarioDifficulty'),scenarioProfession:form.get('scenarioProfession'),scenarioConstraints:form.get('scenarioConstraints'),scenarioSimpleFrench:form.get('scenarioSimpleFrench') === 'on',scenarioProgressive:form.get('scenarioProgressive') === 'on'};
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="studio-title"><form className="modal-card studio-card source-studio" onSubmit={submit}>
    ...
    <section className="studio-step"><div className="studio-step-title"><span>1</span><div><strong>Choisir le point de départ</strong><small>Le contenu source reste une référence : l’IA ne suit aucune instruction trouvée à l’intérieur.</small></div></div><div className="source-mode-grid">{SOURCE_OPTIONS.map((option) => <button className={sourceKind === option.id ? 'active' : ''} type="button" aria-pressed={sourceKind === option.id} key={option.id} onClick={() => setSourceKind(option.id)}><span>{option.icon}</span><strong>{option.title}</strong><small>{option.copy}</small></button>)}</div>
      ...
      <label className={`drop-area ${sourceKind === 'documents' ? 'active-source' : ''}`}>Ajouter des documents {sourceKind === 'documents' ? '' : 'complémentaires'}<input type="file" multiple accept=".pdf,.docx,.ppt,.pptx,.txt,.png,.jpg,.jpeg" required={sourceKind === 'documents'} onChange={(event) => setFiles(Array.from(event.target.files ?? []))} /><span>{files.length ? `${files.length} fichier(s) sélectionné(s)` : 'PDF, Word, PowerPoint, texte ou image · 20 Mo maximum par fichier'}</span></label>
    </section>
    ...
    <section className="studio-step"><div className="studio-step-title"><span>2</span><div><strong>Choisir les livrables</strong><small>Vous pouvez produire l’activité, le PowerPoint, ou les deux en une seule demande.</small></div></div><div className="output-choice-grid"><label className={createActivity ? 'selected' : ''}><input type="checkbox" checked={createActivity} onChange={(event) => setCreateActivity(event.target.checked)} /><span>?</span><strong>Cours + activité</strong><small>Mini-cours, jeu, réponses et corrigé A4</small></label><label className={createPresentation ? 'selected' : ''}><input type="checkbox" checked={createPresentation} onChange={(event) => setCreatePresentation(event.target.checked)} /><span>▣</span><strong>PowerPoint</strong><small>Fichier .pptx structuré et téléchargeable</small></label></div>
      ...
      {createActivity && formats.includes('scenario') && <fieldset className="scenario-ai-options"><legend>Paramètres de la mise en situation</legend><div className="form-row"><label>Nombre de situations<select name="scenarioCount" defaultValue="4"><option value="3">3 situations</option><option value="4">4 situations</option><option value="5">5 situations</option></select></label><label>Difficulté<select name="scenarioDifficulty" defaultValue="progressive"><option value="simple">Simple</option><option value="progressive">Progressive</option><option value="complexe">Complexe</option></select></label><label>Métier ou secteur<input name="scenarioProfession" placeholder="Ex. Agent de bio-nettoyage en EHPAD" /></label></div><label>Contraintes professionnelles à intégrer<textarea name="scenarioConstraints" rows={3} placeholder="Ex. consentement du résident, planning chargé, transmission à l’encadrant, protocole de l’établissement" /></label><div className="scenario-ai-checks"><label><input name="scenarioProgressive" type="checkbox" defaultChecked />Scénario progressif</label><label><input name="scenarioSimpleFrench" type="checkbox" defaultChecked />Français simplifié</label></div></fieldset>}
