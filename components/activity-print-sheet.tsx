...
export function ActivityPrintSheet({ activity }: { activity: PrintableActivity }) {
  ...
  return <article className="print-sheet" aria-hidden="true">
    <header className="print-cover">
      <div className="print-brand"><img src="/icons/progressed-pedago-book-v2-192.png" alt=""/><div><strong>Progressed Pédago</strong><small>Support pédagogique prêt à animer</small></div></div>
...
function ExternalGamePrintSheet({activity}:{activity:PrintableActivity}) {
  ...
  return <article className="print-sheet external-game-print-sheet" aria-hidden="true">
    <header className="print-cover"><div className="print-brand"><img src="/icons/progressed-pedago-book-v2-192.png" alt=""/><div><strong>Progressed Pédago</strong><small>Activité externe et adaptation papier</small></div></div><span className="print-format">Jeu externe intégré</span><p>{activity.theme||'Activité pédagogique'}</p><h1>{activity.title}</h1><div className="print-meta"><span><b>Public</b>{activity.audience||'Adultes en formation'}</span><span><b>Niveau</b>{levelLabel(activity.level)}</span><span><b>Durée</b>{activity.durationMinutes?`${activity.durationMinutes} min`:'À adapter'}</span></div>{paper.includeImages&&game.presentationImageUrl&&<figure className="external-paper-cover-image"><img src={game.presentationImageUrl} alt="Visuel de présentation du jeu"/></figure>}</header>
