...
export function ActivityPrintSheet({ activity }: { activity: PrintableActivity }) {
  ...
  return <article className="print-sheet" aria-hidden="true">
    <header className="print-cover">
      <div className="print-brand"><img src="/icons/progressed-pedago-learning-192.png" alt=""/><div><strong>Progressed Pédago</strong><small>Support pédagogique prêt à animer</small></div></div>
...
function ExternalGamePrintSheet({activity}:{activity:PrintableActivity}) {
  ...
  return <article className="print-sheet external-game-print-sheet" aria-hidden="true">
    <header className="print-cover"><div className="print-brand"><img src="/icons/progressed-pedago-learning-192.png" alt=""/><div><strong>Progressed Pédago</strong><small>Activité externe et adaptation papier</small></div></div><span className="print-format">Jeu externe intégré</span><p>{activity.theme||'Activité pédagogique'}</p><h1>{activity.title}</h1><div className="print-meta"><span><b>Public</b>{activity.audience||'Adultes en formation'}</span><span><b>Niveau</b>{levelLabel(activity.level)}</span><span><b>Durée</b>{activity.durationMinutes?`${activity.durationMinutes} min`:'À adapter'}</span></div>{paper.includeImages&&game.presentationImageUrl&&<figure className="external-paper-cover-image"><img src={game.presentationImageUrl} alt="Visuel de présentation du jeu"/></figure>}</header>
    {paper.learnerVersion&&<><section className="print-identity"><span>Prénom et nom : <i/></span><span>Date : <i/></span><span>Groupe : <i/></span></section><section className="print-block external-paper-introduction"><div><span className="print-number">1</span><div><p className="print-kicker">Version apprenant</p><h2>Avant de commencer</h2></div></div><ul>{(activity.objectives?.length?activity.objectives:['Comprendre et appliquer les notions essentielles du jeu.']).map((objective)=><li key={objective}>{objective}</li>)}</ul>{activity.instructions&&<div className="print-instruction"><strong>Consignes</strong><p>{activity.instructions}</p></div>}{paper.includeExplanations&&<><StructuredExplanation text={game.introduction||game.gameDescription} compact/>{game.preGameExplanation&&<StructuredExplanation text={game.preGameExplanation} compact/>}{game.summary&&<StructuredExplanation text={`## Résumé\n${game.summary}`} compact/>}{game.memo&&<StructuredExplanation text={`## Fiche mémo\n${game.memo}`} compact/>}{game.learnerTips.length>0&&<div className="external-paper-tips"><strong>Conseils</strong><ul>{game.learnerTips.map((tip)=><li key={tip}>{tip}</li>)}</ul></div>}</>}</section><section className="print-block external-paper-access"><div><span className="print-number">2</span><div><p className="print-kicker">Accès au jeu</p><h2>Jouer en ligne</h2></div></div><div className="external-paper-access-grid">{paper.includeQr&&publicUrl&&<figure><img src={qrUrl} alt="QR code vers le jeu externe"/><figcaption>Scannez pour ouvrir le jeu.</figcaption></figure>}<div><strong>Lien sécurisé HTTPS</strong><p>{publicUrl}</p><p>Si le jeu ne s’ouvre pas dans l’application, utilisez ce lien dans votre navigateur.</p>{game.htmlSourceName&&<p><strong>Fichier HTML importé :</strong> {game.htmlSourceName}</p>}{paper.officialFileName&&<p><strong>Support officiel associé :</strong> {paper.officialFileName}</p>}</div></div>{paper.includeImages&&paper.screenshotUrl&&<figure className="external-paper-screenshot"><img src={paper.screenshotUrl} alt="Aperçu autorisé du jeu externe"/><figcaption>Aperçu fourni par le formateur.</figcaption></figure>}</section><section className="print-block print-page-start external-paper-exercises"><div><span className="print-number">3</span><div><p className="print-kicker">Adaptation imprimable</p><h2>Questions et exercices</h2></div></div>{paper.sourceDescription&&<p className="print-adaptation-note">{paper.sourceDescription}</p>}{questions.map((row,index)=><article key={index}><h3><span>{index+1}</span>{row.question}</h3><div className="external-paper-answer-lines"><i/><i/><i/></div>{paper.includeAnswers&&row.answer&&<p className="print-answer"><strong>Réponse :</strong> {row.answer}</p>}</article>)}</section></>}
...
function PrintableMechanic({ type,content }: { type: ActivityType; content: Record<string,unknown> }) {
  ...
  if(type==='audio-quiz'){
    const quiz=normalizeAudioQuizContent(content);
    return <div className="print-question-list">{quiz.items.map((item,index)=><section className="print-question" key={item.id}><h3><span>{index+1}</span>Écoutez la lecture du formateur, puis cochez la bonne réponse.</h3><div className="print-choices">{item.choices.map((choice,choiceIndex)=><p key={choiceIndex}><i>{String.fromCharCode(65+choiceIndex)}</i>{choice}<b/></p>)}</div></section>)}</div>;
  }
...
function PrintableAnswers({ type,content }: { type: ActivityType; content: Record<string,unknown> }) {
  if(type==='audio-quiz'){
    const quiz=normalizeAudioQuizContent(content);
    return <div className="print-answer-list">{quiz.items.map((item,index)=><section key={item.id}><h3>{index+1}. Texte à prononcer : « {item.spokenText} »</h3><p className="print-answer"><strong>Réponse :</strong> {String.fromCharCode(65+item.correctIndex)} — {item.choices[item.correctIndex]}</p>{item.explanation&&<StructuredExplanation text={item.explanation} compact/>}</section>)}</div>;
  }
  if (type === 'quiz' || type === 'tv-quiz') {
