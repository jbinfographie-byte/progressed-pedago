...
export function ManualActivityDialog({ type,onClose,onCreated }: { type: ActivityType; onClose: () => void; onCreated: () => void }) {
  ...
  const [scenario,setScenario] = useState<ScenarioContent>(() => cloneScenario(SCENARIO_EXAMPLE_CONTENT));
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="manual-title"><form className="modal-card studio-card manual-activity-card" onSubmit={submit}>
    ...
    <section className="manual-section"><div className="manual-section-title"><span>1</span><div><strong>Cadre pédagogique</strong><small>Les informations visibles dans le cours et sur le support imprimé.</small></div></div>
      <div className="form-row"><label>Titre<input name="title" required defaultValue={isScenario ? 'Un résident refuse l’entretien de sa chambre' : ''} placeholder="Ex. Prévenir les risques de chute" /></label><label>Thème<input name="theme" required defaultValue={isScenario ? 'Relation avec le résident' : ''} placeholder="Ex. Sécurité au travail" /></label><label>Durée<input name="durationMinutes" type="number" min="1" max="480" defaultValue={isScenario ? '20' : '10'} required /></label></div>
      ...
      <label>Objectifs pédagogiques, un par ligne<textarea name="objectives" rows={3} required defaultValue={isScenario ? 'Adapter son intervention à une situation professionnelle\nRespecter la personne, le protocole et la transmission' : ''} placeholder={'Identifier le risque de chute\nMettre en place un balisage adapté'} /></label>
      <label>Consigne donnée aux participants<textarea name="instructions" rows={2} defaultValue={isScenario ? 'Lisez chaque situation, choisissez une réaction, validez votre décision puis analysez ses conséquences avant de continuer.' : ''} placeholder="Ex. Répondez aux questions puis justifiez chaque choix." /></label>
    </section>
...
function ScenarioEditor({ value,onChange }: { value:ScenarioContent;onChange:(value:ScenarioContent)=>void }) {
  ...
  const removeScene = (index:number) => { if (value.scenes.length <= 3) return; onChange({...value,scenes:linkScenarioScenes(value.scenes.filter((_,sceneIndex) => sceneIndex !== index))}); };
  const addScene = () => { if (value.scenes.length >= 5) return; const number = value.scenes.length + 1; const nextId = `scene-${Date.now()}`; const next:ScenarioScene = {id:nextId,title:`Nouvelle situation ${number}`,location:'Lieu de travail',moment:'Pendant l’intervention',learnerRole:'Professionnel en situation',people:[],context:'Décrivez ce qui se passe concrètement.',problem:'Précisez le problème à résoudre.',constraints:[],mission:'Choisir une réaction et justifier sa décision.',objective:'Adapter sa conduite à la situation.',competencies:[],observationCriteria:[],usefulInformation:[],documents:[],question:'Quelle réaction professionnelle choisissez-vous ?',choices:[0,1,2].map((score) => ({id:`choice-${Date.now()}-${score}`,text:`Décision ${score + 1}`,score:score as 0|1|2,consequence:'Décrivez la conséquence observable.',positivePoints:[],risks:[],recommendedConduct:'Précisez la conduite professionnelle recommandée.',explanation:'Expliquez pourquoi cette décision est adaptée ou non.',nextSceneId:null}))}; onChange({...value,scenes:linkScenarioScenes([...value.scenes,next])}); };
  return <div className="scenario-editor"><div className="scenario-editor-note"><span>4</span><p><strong>Modèle métier prêt à adapter</strong>Chaque scène comporte un contexte, une mission, 3 à 4 décisions plausibles et un retour noté sur 2. Vous pouvez ajouter, déplacer ou supprimer les scènes sans toucher au code.</p></div>{value.scenes.map((scene,index) => <details className="scenario-scene-editor" open={index === 0} key={scene.id}><summary><span>Situation {index + 1}</span><strong>{scene.title}</strong><small>{scene.choices.length} décisions</small></summary><div className="scenario-scene-editor-body"><div className="scenario-editor-actions"><button type="button" disabled={index === 0} onClick={() => moveScene(index,-1)}>↑ Monter</button><button type="button" disabled={index === value.scenes.length - 1} onClick={() => moveScene(index,1)}>↓ Descendre</button><button type="button" disabled={value.scenes.length <= 3} onClick={() => removeScene(index)}>Supprimer</button></div><div className="form-row two"><label>Titre de la situation<input value={scene.title} onChange={(event) => updateScene(index,{title:event.target.value})} /></label><label>Lieu<input value={scene.location} onChange={(event) => updateScene(index,{location:event.target.value})} /></label><label>Moment<input value={scene.moment} onChange={(event) => updateScene(index,{moment:event.target.value})} /></label><label>Rôle de l’apprenant<input value={scene.learnerRole} onChange={(event) => updateScene(index,{learnerRole:event.target.value})} /></label></div><label>Contexte<textarea rows={3} value={scene.context} onChange={(event) => updateScene(index,{context:event.target.value})} /></label><label>Problème à résoudre<textarea rows={2} value={scene.problem} onChange={(event) => updateScene(index,{problem:event.target.value})} /></label><div className="form-row two"><label>Mission<textarea rows={2} value={scene.mission} onChange={(event) => updateScene(index,{mission:event.target.value})} /></label><label>Objectif pédagogique<textarea rows={2} value={scene.objective} onChange={(event) => updateScene(index,{objective:event.target.value})} /></label></div><div className="form-row two"><LineList label="Compétences, une par ligne" value={scene.competencies} onChange={(competencies) => updateScene(index,{competencies})} /><LineList label="Points à observer, un par ligne" value={scene.observationCriteria} onChange={(observationCriteria) => updateScene(index,{observationCriteria})} /></div><label>Question posée à l’apprenant<input value={scene.question} onChange={(event) => updateScene(index,{question:event.target.value})} /></label><div className="scenario-choice-editors">{scene.choices.map((choice,choiceIndex) => <article key={choice.id}><header><strong>Décision {String.fromCharCode(65 + choiceIndex)}</strong><label>Score<select value={choice.score} onChange={(event) => updateScene(index,{choices:scene.choices.map((item,itemIndex) => itemIndex === choiceIndex ? {...item,score:Number(event.target.value) as 0|1|2} : item)})}><option value="0">0 · à risque</option><option value="1">1 · partielle</option><option value="2">2 · recommandée</option></select></label></header><label>Réponse proposée<textarea rows={2} value={choice.text} onChange={(event) => updateScene(index,{choices:scene.choices.map((item,itemIndex) => itemIndex === choiceIndex ? {...item,text:event.target.value} : item)})} /></label><label>Conséquence<textarea rows={2} value={choice.consequence} onChange={(event) => updateScene(index,{choices:scene.choices.map((item,itemIndex) => itemIndex === choiceIndex ? {...item,consequence:event.target.value} : item)})} /></label><label>Conduite recommandée<textarea rows={2} value={choice.recommendedConduct} onChange={(event) => updateScene(index,{choices:scene.choices.map((item,itemIndex) => itemIndex === choiceIndex ? {...item,recommendedConduct:event.target.value} : item)})} /></label><label>Explication pédagogique<textarea rows={2} value={choice.explanation} onChange={(event) => updateScene(index,{choices:scene.choices.map((item,itemIndex) => itemIndex === choiceIndex ? {...item,explanation:event.target.value} : item)})} /></label>{scene.choices.length > 3 && <button className="scenario-remove-choice" type="button" onClick={() => updateScene(index,{choices:scene.choices.filter((_,itemIndex) => itemIndex !== choiceIndex)})}>Retirer cette décision</button>}</article>)}</div>{scene.choices.length < 4 && <button className="add-question" type="button" onClick={() => updateScene(index,{choices:[...scene.choices,{id:`choice-${Date.now()}`,text:'Nouvelle décision',score:1,consequence:'Décrivez la conséquence.',positivePoints:[],risks:[],recommendedConduct:'Précisez la conduite recommandée.',explanation:'Expliquez le raisonnement professionnel.',nextSceneId:null}]})}>＋ Ajouter une décision</button>}</div></details>)}<button className="add-question" type="button" disabled={value.scenes.length >= 5} onClick={addScene}>＋ Ajouter une situation ({value.scenes.length}/5)</button><section className="scenario-debrief-editor"><strong>Bilan final</strong><label>Synthèse<textarea rows={3} value={value.debrief.summary} onChange={(event) => onChange({...value,debrief:{...value.debrief,summary:event.target.value}})} /></label><LineList label="Bonnes pratiques, une par ligne" value={value.debrief.bestPractices} onChange={(bestPractices) => onChange({...value,debrief:{...value.debrief,bestPractices}})} /></section></div>;
}
