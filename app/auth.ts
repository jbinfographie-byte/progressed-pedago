...
export function validCode(code:string){return /^\d{6}$/.test(code)}
...
export async function hashCode(code:string,salt:string){
  ...
  const bits=await crypto.subtle.deriveBits({name:"PBKDF2",hash:"SHA-256",salt:hexToBytes(salt),iterations:120000},key,256);
...
export async function getTrainer(request:Request){
  ...
  const rows=await getDb().select({id:trainers.id,email:trainers.email,sessionId:trainerSessions.id}).from(trainerSessions).innerJoin(trainers,eq(trainerSessions.trainerId,trainers.id)).where(and(eq(trainerSessions.id,id),gt(trainerSessions.expiresAt,now))).limit(1);
  return rows[0]||null;
}
