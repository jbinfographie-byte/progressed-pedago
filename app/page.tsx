...
export default function Home(){
  ...
  useEffect(()=>{if(!printable)return;const previousTitle=document.title;document.title=`Activite-${printable.title.replace(/[^a-zA-Z0-9À-ÿ]+/g,"-")}`;const finishPrint=()=>{document.title=previousTitle;setPrintable(null)};window.addEventListener("afterprint",finishPrint,{once:true});const timer=window.setTimeout(()=>window.print(),180);return()=>{window.clearTimeout(timer);window.removeEventListener("afterprint",finishPrint);document.title=previousTitle}},[printable]);
  ...
  </div>{printable&&<PrintableActivity activity={printable}/>}</>
}
