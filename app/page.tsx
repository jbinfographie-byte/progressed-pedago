...
function LoginScreen({onAuthenticated}:{onAuthenticated:(trainer:Trainer)=>void}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{const response=await fetch(`/api/auth/${mode==="login"?"login":"register"}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:data.get("email"),password})});const body=await response.json();if(!response.ok)throw new Error(body.error||"Accès refusé.");onAuthenticated(body.trainer)}catch(reason){setError(reason instanceof Error?reason.message:"Accès refusé.")}finally{setLoading(false)}
  }
  return <main className="login-page">
    ...
    <section className="login-panel"><div className="login-card"><div className="login-icon"><Icon name="lock" size={25}/></div><span className="kicker">{mode==="login"?"BON RETOUR":"PREMIER ACCÈS"}</span><h2>{mode==="login"?"Ouvrir mon espace":"Créer mon accès formateur"}</h2><p>{mode==="login"?"Saisissez votre e-mail et votre mot de passe.":"Choisissez librement un mot de passe long et personnel."}</p><form onSubmit={submit}><label>Adresse e-mail<input type="email" name="email" autoComplete="email" placeholder="formateur@exemple.fr" required/></label><label>Mot de passe<input type="password" name="password" value={password} onChange={e=>setPassword(e.target.value)} minLength={mode==="register"?12:1} maxLength={128} autoComplete={mode==="login"?"current-password":"new-password"} placeholder="Votre mot de passe" required/>{mode==="register"&&<><div className={`password-strength s${passwordStrength(password)}`}><i/><i/><i/><i/></div><small>12 caractères minimum. Une phrase de passe longue est recommandée.</small></>}</label>{mode==="register"&&<label>Confirmer le mot de passe<input type="password" name="confirmation" minLength={12} maxLength={128} autoComplete="new-password" placeholder="Retapez votre mot de passe" required/></label>}{error&&<div className="auth-error">{error}</div>}<button className="auth-submit" disabled={loading}>{loading?<><span className="loader"/>Vérification…</>:mode==="login"?<>Accéder à mon espace <span>→</span></>:<>Créer mon accès <span>→</span></>}</button></form><div className="auth-switch"><span>{mode==="login"?"Première visite ?":"Vous avez déjà un accès ?"}</span><button onClick={()=>{setMode(mode==="login"?"register":"login");setPassword("");setError("")}}>{mode==="login"?"Créer un accès formateur":"Me connecter"}</button></div><small className="privacy-note"><Icon name="lock" size={12}/> Votre mot de passe est chiffré et n’est jamais affiché.</small></div>
    </section>
  </main>
}
...
type AdminAccount={id:number;email:string;role:string;status:string;failedAttempts:number;lockedUntil:string|null;lastLoginAt:string|null;mustChangePassword:boolean;createdAt:string};
type AdminEvent={id:number;email:string;event:string;detail:string|null;createdAt:string};
function AdminPanel(){
  const [accounts,setAccounts]=useState<AdminAccount[]>([]);const [events,setEvents]=useState<AdminEvent[]>([]);const [registration,setRegistration]=useState(true);const [loading,setLoading]=useState(true);const [message,setMessage]=useState("");
  async function load(){setLoading(true);const response=await fetch("/api/admin");const data=await response.json();if(response.ok){setAccounts(data.accounts);setEvents(data.events);setRegistration(data.settings.registrationEnabled)}else setMessage(data.error||"Chargement impossible");setLoading(false)}
  useEffect(()=>{load()},[]);
