...
function Player({activity,step,selected,setSelected,finish,score,next,close,save,saved,generateLesson}:{activity:Activity;step:number;selected:number|null;setSelected:(n:number)=>void;finish:boolean;score:number;next:()=>void;close:()=>void;save:(e:FormEvent<HTMLFormElement>)=>void;saved:boolean;generateLesson:(activity:Activity)=>Promise<Lesson>}){
  ...
  const [lesson,setLesson]=useState(activity.lesson||null);const [showLesson,setShowLesson]=useState(false);const [lessonLoading,setLessonLoading]=useState(false);const [lessonError,setLessonError]=useState("");
  ...
  return <div className="modal-backdrop play-bg" onMouseDown={leave}><section className="modal play-modal" onMouseDown={e=>e.stopPropagation()} role="dialog" aria-modal="true"><button className="modal-close" onClick={leave}>×</button>
    {!finish?<div className={`play-stage ${activity.coverImageUrl?"has-visual":""}`}><div className="play-head"><span className="question-tag mint">{activity.type}</span><div><span>{step+1} / {activity.questions.length}</span><button className="lesson-toggle" onClick={toggleLesson} disabled={lessonLoading}><Icon name="book" size={16}/>{lessonLoading?"Génération…":showLesson?"Masquer le cours":lesson?"Voir le cours":"Générer le cours avec l’IA"}</button><button onClick={expand} title="Afficher en plein écran"><Icon name="expand" size={16}/>Plein écran</button></div></div><div className="progress"><i style={{width:`${(step+1)/activity.questions.length*100}%`}}/></div>{showLesson&&<section className="learner-lesson"><div className="lesson-heading"><span><Icon name="wand" size={17}/></span><div><small>COURS ET EXPLICATIONS</small><h2>{lesson?.title||"Création du cours en cours…"}</h2></div><button onClick={()=>setShowLesson(false)}>Continuer l’exercice →</button></div>{lessonLoading?<div className="lesson-loading"><span className="loader dark"/>L’intelligence artificielle prépare une explication simple et structurée…</div>:lesson?<><p className="lesson-summary">{lesson.summary}</p><div className="lesson-columns"><div><h3>Points essentiels</h3><ul>{lesson.keyPoints.map(point=><li key={point}>{point}</li>)}</ul></div><div><h3>Méthode à retenir</h3><ol>{lesson.steps.map(item=><li key={item}>{item}</li>)}</ol></div></div><div className="lesson-tip"><b>Conseil pour réussir</b><p>{lesson.learnerTip}</p></div></>:null}{lessonError&&<div className="lesson-error">{lessonError}<button onClick={toggleLesson}>Réessayer</button></div>}</section>}{activity.coverImageUrl&&<img className="quiz-image" src={activity.coverImageUrl} alt={activity.imageAlt||`Illustration de ${activity.title}`}/>}<div className="question-zone">{q.objective&&<p className="question-objective">OBJECTIF : {q.objective}</p>}<h2>{q.question}</h2><div className="answers">{q.options.map((o,i)=><button className={selected===i?(i===q.correct?"correct":"wrong"):""} key={o} onClick={()=>setSelected(i)}><span>{String.fromCharCode(65+i)}</span>{o}</button>)}</div>{selected!==null&&<><div className={`feedback ${selected===q.correct?"good":"retry"}`}><strong>{selected===q.correct?"Bonne réponse !":"Pas tout à fait"} — Explication</strong><p>{q.explanation}</p></div>{q.sourceIndexes?.length?<div className="question-sources"><b>Sources :</b>{q.sourceIndexes.map(i=>activity.sources?.[i]).filter(Boolean).map((s,i)=><a key={s!.url} href={s!.url} target="_blank" rel="noreferrer">{i+1}. {s!.publisher||s!.title}</a>)}</div>:null}</>}<button className="next-btn" disabled={selected===null} onClick={next}>{step===activity.questions.length-1?"Voir mon résultat":"Question suivante"} →</button></div></div>:
