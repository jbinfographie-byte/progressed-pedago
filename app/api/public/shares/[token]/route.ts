...
export async function GET(request:Request,context:{params:Promise<{token:string}>}){
  ...
  try{
    ...
    return jsonOk({share:{shortCode:share.shortCode,mode:share.mode,identityMode:share.identityMode,expiresAt:share.expiresAt,sessionOpen:share.sessionOpen},training:{id:training.id,name:training.name,description:training.description,audience:training.audience,level:training.level,durationMinutes:training.durationMinutes,coverImageUrl:training.coverImageUrl},path:{id:path.id,name:path.name,items:items.map((item)=>({...item,objectives:JSON.parse(item.objectivesJson),content:JSON.parse(item.contentJson),sources:JSON.parse(item.sourcesJson),objectivesJson:undefined,contentJson:undefined,sourcesJson:undefined}))},resources:resources.map((resource)=>({...resource,metadata:JSON.parse(resource.metadataJson),metadataJson:undefined})),participant:participant?{id:participant.id,displayName:participant.displayName,progressPercent:participant.progressPercent,lastPathItemId:participant.lastPathItemId,progress}:null});
