...
export async function POST(request:Request,context:{params:Promise<{token:string}>}){
  try{
    ...
    const body=await readAiJson(request);if(body.transcriptConsent!==true)throw new AppError(400,'Le bilan détaillé nécessite votre accord pour analyser temporairement la transcription.','TRANSCRIPT_CONSENT_REQUIRED');
    ...
    const credential=await resolveOpenAiCredential(share.trainerId);
    const apiKey=credential.apiKey;const config=normalizeVoiceCoachContent(JSON.parse(item.contentJson));
    ...
    if(!response.ok)throw new AppError(response.status>=500?503:502,'Le bilan vocal n’a pas pu être préparé. Réessayez dans quelques instants.','VOICE_SUMMARY_ERROR');
