...
import { courseImagesFromContent } from '@/lib/course-images';
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try { assertSameOrigin(request); const user = await requirePermission('deleteActivities'); const id = (await context.params).id; const row = await owned(id, user.id); const deleteFiles=new URL(request.url).searchParams.get('deleteFiles') === 'true'; const courseImages=courseImagesFromContent(JSON.parse(row.contentJson)); if(deleteFiles){const keys=[row.imageObjectKey,...courseImages.map((image)=>image.objectKey)].filter((key):key is string=>Boolean(key));if(keys.length)await env.FILES.delete(keys);} await getDb().delete(activities).where(and(eq(activities.id, id), eq(activities.trainerId, user.id))); await audit(user.id, 'activity.deleted', 'activity', id, { filesDeleted: deleteFiles&&(Boolean(row.imageObjectKey)||courseImages.length>0) }, request); return jsonOk({ message: 'L’activité a été supprimée de votre bibliothèque.' }); } catch (error) { return jsonError(error); }
}
