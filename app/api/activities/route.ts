...
import { ActivityDraft, isCreatableActivityType, isManualActivityType, validateActivityDraft } from '@/lib/activity-types';
...
export async function POST(request: Request) {
  try {
    ...
    if (!isCreatableActivityType(draft.type) && !isManualActivityType(draft.type)) throw new AppError(400, 'Ce format n’est pas disponible dans l’éditeur guidé.', 'ACTIVITY_TYPE_NOT_CREATABLE');
