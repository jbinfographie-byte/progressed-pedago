...
export async function POST(request:Request){
  ...
  try{
    ...
    if(invalid)return Response.json({error:`Le fichier « ${invalid.name} » n’est pas accepté. Formats autorisés : PDF, PNG et JPEG.`},{status:415,headers:noStore});
    ...
    content.push({type:"input_text",text:`Analyse conjointement les ${files.length} sources fournies pour construire une base pédagogique unique. Analyse chaque PDF de la première à la dernière page, y compris lorsqu’il résulte de plusieurs PDF fusionnés. Considère chaque image PNG ou JPEG comme une page autonome. Crée exactement une fiche par page ou image et conserve l’ordre des sources. sourceIndex commence à 1 et correspond au numéro SOURCE indiqué avant chaque fichier ; sourceName reprend son nom exact ; pageNumber recommence à 1 pour chaque PDF et vaut 1 pour une image. Pour chaque fiche, donne un titre court, un résumé fidèle en français simple, 2 à 5 points importants et indique si elle est pédagogiquement exploitable. Synthétise ensuite le thème commun, 2 à 4 publics plausibles et 3 à 6 objectifs pédagogiques avec des verbes d’action. Ne génère encore aucun cours ni exercice. Signale les contradictions, doublons, pages illisibles ou documents sans rapport dans warnings.`});
    ...
    return Response.json({analysis,files:files.map(file=>({name:file.name,size:file.size,type:file.type})),totalBytes},{headers:noStore});
