...
export async function POST(request:Request){
  ...
  try{
    ...
    const trainerPrompt=String(form.get("prompt")||"").trim();
    if(!theme&&!trainerPrompt&&!files.length)return Response.json({error:"Ajoutez un thème, une consigne ou un document à analyser."},{status:400});
    ...
    const invalid=files.find(file=>!Object.values(extensionTypes).includes(normalizedType(file)));if(invalid)return Response.json({error:`Le fichier « ${invalid.name} » n’est pas accepté. Utilisez un PDF, Word DOCX, TXT, PNG ou JPEG.`},{status:415});
    ...
    if(files.length&&env.BUCKET){
      for(const file of files){const mime=normalizedType(file);const extension=file.name.toLowerCase().split(".").pop()||"bin";const key=`course-sources/${crypto.randomUUID()}.${extension}`;await env.BUCKET.put(key,await file.arrayBuffer(),{httpMetadata:{contentType:mime,contentDisposition:`inline; filename="${file.name.replace(/["\\]/g,"")}"`},customMetadata:{originalName:file.name.slice(0,120)}});sourceDocuments.push({name:file.name,key,size:file.size,url:`/api/media/${encodeURIComponent(key)}`,type:mime})}
      ...
      generated.sources=[...sourceDocuments.map(document=>({title:document.name,url:document.url,publisher:document.type==="application/pdf"?"PDF importé":document.type.startsWith("image/")?"Image importée":"Document importé"})),...(generated.sources||[])];
    }
