...
export async function POST(request:Request){
  ...
  try{
    ...
    if(secrets.OPENAI_API_KEY){
      ...
      const prompt=`Crée un cours complet pour adultes en formation professionnelle. Analyse d’abord avec fidélité le PDF joint s’il existe. Ne déforme pas le document et signale implicitement ses limites en restant dans son périmètre. Complète uniquement avec des sources institutionnelles fiables si la recherche est activée. Structure le résultat avec 4 à 8 sections développées, 1 à 3 tableaux utiles, 1 à 3 schémas de processus, 2 à 4 exercices d’application et exactement ${count} questions de quiz. Chaque quiz doit avoir 3 ou 4 choix, une seule bonne réponse, des distracteurs plausibles et une explication. Mélange la position des bonnes réponses. Français clair, adulte et professionnel.\n\nTHÈME : ${theme||"À déterminer à partir du PDF"}\nPUBLIC : ${form.get("audience")||"Adultes en formation professionnelle"}\nOBJECTIFS : ${form.get("objective")||"Identifier, comprendre et appliquer les notions essentielles"}\nDURÉE VISÉE : ${form.get("duration")||60} minutes\nCONSIGNES DU FORMATEUR : ${form.get("prompt")||"Cours concret, progressif et directement utilisable"}`;
    ...
    }else generated=fallbackCourse(theme,count,pdf?.name);
    ...
    return Response.json({activity:{...generated,type:"Cours complet"},mode,notice:mode==="demo"?"Cours de démonstration créé. Configurez OpenAI pour analyser réellement le PDF.":undefined});
