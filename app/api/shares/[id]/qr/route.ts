...
export async function GET(request:Request,context:{params:Promise<{id:string}>}){
  try{
    ...
    page.drawRectangle({x:0,y:0,width:595.28,height:841.89,color:rgb(248/255,248/255,242/255)});page.drawRectangle({x:0,y:732,width:595.28,height:110,color:dark});page.drawText('PROGRESSED PEDAGO',{x:44,y:798,size:12,font:bold,color:lime});page.drawText(share.mode==='classroom'?'Rejoignez le cours en direct':'Votre travail a domicile',{x:44,y:756,size:27,font:bold,color:rgb(1,1,1)});
    page.drawText('1. Scannez le QR code avec votre telephone.',{x:54,y:682,size:14,font:regular,color:dark});page.drawText('2. Renseignez uniquement les informations demandees.',{x:54,y:654,size:14,font:regular,color:dark});page.drawText(share.mode==='classroom'?'3. Ouvrez l activite choisie par votre formateur.':'3. Avancez a votre rythme et conservez votre code de reprise.',{x:54,y:626,size:14,font:regular,color:dark});
    page.drawImage(image,{x:147.5,y:285,width:300,height:300});page.drawText(`Code court : ${share.shortCode}`,{x:208,y:246,size:16,font:bold,color:dark});page.drawText(share.mode==='classroom'?'Activite ouverte et animee par le formateur':'Parcours complet en autonomie',{x:175,y:216,size:12,font:regular,color:dark});
