...
export async function POST(request: Request) {
  try {
    ...
    const content: Array<Record<string, unknown>> = [{ type: 'input_text', text: buildPrompt(prompt, formats, body) }];
    let totalBytes = 0;
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({ model: credential.model, store: false, instructions: 'Tu es un ingénieur pédagogique francophone. Produis des contenus sûrs, concrets, sans ambiguïté, adaptés à la formation professionnelle adulte. N’invente jamais de source. Réponds strictement selon le schéma.', input: [{ role: 'user', content }], tools: research ? [{ type: 'web_search' }] : undefined, include: research ? ['web_search_call.action.sources'] : undefined, text: { format: { type: 'json_schema', name: 'progressed_pedago_bundle', strict: true, schema: bundleSchema(formats) } }, max_output_tokens: 12000 }),
    });
    ...
    for (const draft of drafts) { const validation = validateActivityDraft(draft); if (!validation.valid) throw new AppError(502, `Contrôle qualité : ${validation.errors.join(' ')}`, 'OPENAI_QUALITY_REJECTED'); }
...
function buildPrompt(prompt: string, formats: ActivityType[], body: Record<string, unknown>) { return `Demande du formateur : ${prompt}\nPublic : ${String(body.audience ?? 'adultes en formation professionnelle')}\nNiveau : ${String(body.level ?? 'débutant')}\nDurée : ${String(body.durationMinutes ?? 20)} minutes\nFormats obligatoires : ${formats.join(', ')}.\nPour chaque activité, crée une mécanique réellement compatible avec le format : questions/choices/correctIndex pour quiz, statements pour vrai-faux, items et zones pour glisser-déposer, grid et words pour mots mêlés, cards recto-verso pour cartes, sectors pour roues, categories et items pour classement. Place cette mécanique sous forme de chaîne JSON valide dans contentJson. Une roue du défi doit contenir des missions réalisables. Mélange les bonnes réponses. Explique chaque correction. Les sources doivent provenir uniquement de la recherche web ou des documents joints.`; }
