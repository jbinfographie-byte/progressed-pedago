...
export async function POST(request: Request) {
  try {
    ...
    const multiPageCourse = fileIds.length > 0 || sourceKind === 'external' && externalOutputs.includes('course');
    ...
    const answerOrderSeed=crypto.randomUUID();
    const stabilizedCoursePages=multiPageCourse?stabilizeCoursePages(parsed.coursePages??parsed.activities[0]?.coursePages,coursePageCount,allowedPages):[];
    const sharedCoursePages=stabilizedCoursePages.length
      ? diversifyGeneratedAnswerPositions({coursePages:stabilizedCoursePages},`${answerOrderSeed}:course`).coursePages as typeof stabilizedCoursePages
      : stabilizedCoursePages;
    let drafts = parsed.activities.map((item,itemIndex) => { let mechanic: Record<string, unknown>; try { mechanic = JSON.parse(item.contentJson) as Record<string, unknown>; } catch { throw new AppError(502, `La mécanique de « ${item.title} » est invalide.`, 'OPENAI_INVALID_MECHANIC'); } const verifiedSources = (item.sources ?? []).filter((candidate) => { try { const url = new URL(candidate.url); return url.protocol === 'https:' && (!research || citedUrls.has(candidate.url)); } catch { return false; } }); const userSource = source ? { title:source.title,organization:source.organization,url:source.url,usedFor:source.analysisMethod === 'public_metadata_visuals' ? 'Titre, description et aperçus publics de la vidéo restreinte' : source.kind === 'video' ? 'Audio, images et contenu de la vidéo' : 'Contenu de la page fournie' } : null; const sources = userSource && !verifiedSources.some((candidate) => candidate.url === userSource.url) ? [...verifiedSources,userSource,...documentSources] : [...verifiedSources,...documentSources]; const variedMechanic=diversifyGeneratedAnswerPositions(mechanic,`${answerOrderSeed}:${itemIndex}:${item.type}`); const enrichedMechanic = { ...variedMechanic,...(source?.media?{sourceMedia:source.media}:{}),...(sharedCoursePages.length?{coursePages:sharedCoursePages,courseSettings:{length:courseLength,pageCount:coursePageCount,sourcePageCount:knowledgeRows.length}}:{}) }; return { ...item, sources, content: enrichedMechanic } as ActivityDraft; });
...
function buildExternalResourceDraft(body:Record<string,unknown>,sourceUrl:string,coursePages:unknown[]):ActivityDraft {
  ...
  const level=['debutant','intermediaire','avance'].includes(String(analysis.level))?String(analysis.level) as ActivityDraft['level']:'debutant';
  const objectives=Array.isArray(analysis.objectives)?analysis.objectives.map(String).map((item)=>item.trim()).filter(Boolean).slice(0,12):[];
  const explanation=[analysis.introduction,analysis.preGameExplanation,analysis.summary,analysis.memo].map((value)=>String(value??'').trim()).filter(Boolean).join('\n\n');
  const content={...game,...(coursePages.length?{coursePages,courseSettings:{length:String(body.courseLength??'intermediate'),pageCount:coursePages.length,sourcePageCount:0}}:{})};
  return {type:'external-game',title,theme,audience,level,objectives:objectives.length?objectives:['Exploiter la ressource externe dans une activité guidée'],durationMinutes:Math.min(480,Math.max(1,Number(body.durationMinutes)||20)),instructions:String(analysis.instructions??'Consultez la ressource, réalisez l’activité puis validez votre progression.'),explanation,correction:String(analysis.correction??''),sources:[{title,organization:providerLabel(game.provider),url:game.sourceUrl,usedFor:'Activité externe analysée et intégrée'}],content};
}

function externalActivityTypes(outputs:string[]):ActivityType[] { const types:ActivityType[]=[];if(outputs.includes('quiz'))types.push('quiz');if(outputs.includes('true-false'))types.push('true-false');if(outputs.includes('scenario')||outputs.includes('case-study'))types.push('scenario');if(outputs.includes('exercise'))types.push('drag-drop');return [...new Set(types)]; }
