...
export async function POST(request: Request) {
  try {
    ...
    return jsonOk({ activities: created.map(({ id, draft }) => ({ id, title: draft.title, type: draft.type })), message: `${created.length} création(s) contrôlée(s) et enregistrée(s) dans votre bibliothèque.${source?.analysisMethod === 'audio_transcription' ? ' La piste audio de la vidéo a été transcrite automatiquement.' : ''}`, sourceTranscript:source?.transcript ?? null }, 201);
