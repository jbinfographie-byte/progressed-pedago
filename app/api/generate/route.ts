...
export async function POST(request: Request) {
  try {
    assertSameOrigin(request); const user = await requirePermission('useAi'); assertPermission(user, 'createActivities'); const body = await readJson(request); const fileIds = Array.isArray(body.fileIds) ? body.fileIds.map(String).slice(0, 5) : []; const source = await resolveSourceMaterial(body); const sourceKind = normalizeSourceKind(body.sourceKind); const rawPrompt = String(body.prompt ?? '').trim();
    ...
    await audit(user.id, 'ai.bundle_generated', 'activity_bundle', null, { count: created.length, formats, fileCount: files.length, research, sourceKind }, request);
