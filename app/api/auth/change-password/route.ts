...
export async function POST(request:Request){
  try{
    ...
    if(!validPassword(newPassword))return Response.json({error:"Le nouveau mot de passe doit contenir au moins 12 caractères, une majuscule et un chiffre."},{status:400});
    ...
    const salt=makeSalt();await getDb().update(trainers).set({codeSalt:salt,codeHash:await hashCode(newPassword,salt),passwordVersion:3,mustChangePassword:false,failedAttempts:0,lockedUntil:null}).where(eq(trainers.id,trainer.id));
