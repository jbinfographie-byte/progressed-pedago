...
export async function POST(request:Request){
  try{
    ...
    if(exists.length)return Response.json({error:exists[0].emailVerified?"Un accès existe déjà pour cette adresse.":"Votre demande est déjà enregistrée. Saisissez le code unique remis par l’administrateur.",verificationRequired:!exists[0].emailVerified,email},{status:409});
    ...
    return Response.json({verificationRequired:true,email,message:notification.ok?"Votre demande a été envoyée à l’administrateur. Saisissez ici le code unique qu’il vous remettra.":"Votre demande est enregistrée et visible par l’administrateur. Saisissez ici le code unique qu’il vous remettra."},{status:202});
