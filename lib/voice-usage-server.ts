...
export async function finalizeVoiceUsage(input: {
  ...
  const audioSeconds = normalizeVoiceUsageSeconds({ reportedSeconds: input.durationSeconds, startedAtSeconds: started.createdAt, maximumSeconds: config.maxDurationMinutes * 60 });
