...
const list=(value:unknown)=>Array.isArray(value)?value.map(String).map((item)=>item.trim()).filter(Boolean).slice(0,40):[];
...
export function normalizeVoiceCoachContent(value:unknown):VoiceCoachContent {
  ...
  return {
    ...
    vocabulary:list(source.vocabulary),phrases:list(source.phrases),objectives:list(source.objectives),speed,correctionLevel:correction,conversationMode:mode,
...
export function voiceCoachInstructions(config:VoiceCoachContent):string {
  ...
  return `Tu es le coach vocal patient d'un apprenant adulte. Langue travaillée : ${config.learningLanguage}. Langue des explications : ${config.explanationLanguage}. Niveau CECRL : ${config.cefrLevel}. Thème : ${config.topic}. Mode : ${mode}. Objectifs : ${config.objectives.join(' ; ')||'oser parler et progresser'}. Vocabulaire cible : ${config.vocabulary.join(', ')||'à choisir selon le thème'}. Phrases cibles : ${config.phrases.join(' ; ')||'à construire progressivement'}. Niveau de correction : ${config.correctionLevel}. ${config.trainerPrompt}
Règles impératives : donne une seule consigne à la fois ; attends réellement la réponse orale ; adapte la difficulté au niveau observé ; corrige une difficulté prioritaire à la fois ; explique brièvement dans la langue d'explication ; fais répéter sans humilier ; encourage de manière précise ; ne prétends jamais mesurer une prononciation que tu n'as pas entendue. Commence par une salutation très courte puis une question simple liée au thème.`;
}
