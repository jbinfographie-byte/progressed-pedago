...
export type AuthUser = { id: string; email: string; displayName: string | null; role: 'admin' | 'trainer'; status: 'pending' | 'active' | 'suspended' | 'revoked' };
...
export async function getCurrentUser(): Promise<AuthUser | null> {
  ...
  const rows = await getDb().select({ id: users.id, email: users.email, displayName: users.displayName, role: users.role, status: users.status })
